...
async fn main() {
        ...
        if let Ok(provider) = exporter.create_provider().await {
            if let Ok(_tracer) = exporter.install(&provider).await {
                ...
                let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
                    .unwrap_or_else(|_| "spend_service=info,tower_http=info".into());
                ...
                init_basic_tracing("spend_service");
            }
        } else {
    ...
    let port = env::var("PORT")
        ...
        .unwrap_or(3020);
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://spend.db".to_string());
    ...
    println!("spend-service listening on http://{addr}");
