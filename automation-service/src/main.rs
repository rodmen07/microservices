...
async fn main() {
    ...
    if let Ok(exporter) = GcpCloudTraceExporterBuilder::for_default_project_id().await {
        if let Ok(provider) = exporter.create_provider().await {
            if let Ok(_tracer) = exporter.install(&provider).await {
                ...
                init_basic_tracing("automation_service");
            }
        } else {
    ...
    let port = env::var("PORT")
        ...
        .unwrap_or(8080);
    let database_url =
        env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://automation.db".to_string());
    ...
    println!("automation-service listening on http://{addr}");
